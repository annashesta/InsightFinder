...
**Главный принцип: Все утверждения в отчете ДОЛЖНЫ иметь подтверждение в предоставленных данных. Запрещено выдумывать данные, дополнять своими или ссылаться на несуществующие графики. Если данных для какого-либо раздела недостаточно или они отсутствуют — полностью пропусти этот пункт.**
...
**ВАЖНО:** Ссылайся на графики по их названию (например, "см. график boxplot_MonthlyRevenue"). Не вставляй изображения в отчет. Пользователь сам откроет файлы графиков.
...
*   Назови признак, который *наиболее сильно* различает группы согласно результатам анализа. Приведи конкретные значения (например, среднее значение признака в группе 1: X, в группе 0: Y). Проинтерпретируй, что это может означать в контексте исследования. Обязательно сошлись на соответствующий график.

### 2. Анализ корреляций
*   **{correlation_summary}**
*   **{correlation_details}**
*   Выдели только самые сильные и статистически значимые корреляции, имеющие отношение к целевой переменной.
*   Проинтерпретируй, что эти корреляции могут означать в контексте исследования. Обязательно сошлись на соответствующие графики.

### 3. Сравнительный анализ статистик
*   **{descriptive_stats_summary}**
*   **{stats_details}**
*   Сравни основные метрики (среднее, медиана, стандартное отклонение и прочие) по ключевым непрерывным признакам между группами. Акцентируй внимание на признаках с наибольшей разницей. Проинтерпретируй, что эти различия могут означать. Обязательно сошлись на соответствующие графики.
...
*   **{outlier_summary}**
*   **{outlier_details}**
*   Сообщите о наличии выбросов, если они обнаружены. Укажите, в какой группе и на каких признаках они сконцентрированы, и как это может повлиять на анализ.

### 7. Анализ взаимодействия признаков
*   **{interaction_summary}**
*   **{interaction_details}**
*   Опиши, если были обнаружены признаки, чье влияние на целевую переменную усиливается/ослабевает в зависимости от других факторов. Если таких признаков нет или данные отсутствуют — пропусти этот пункт полностью.

### 8. Важность признаков (модель)
*   **{full_model_summary}**
*   **{model_details}**
*   Ранжируй признаки по их важности в предсказании принадлежности к группе, согласно результатам ML-модели. Проинтерпретируй, что это может означать. Обязательно сошлись на соответствующие графики.
