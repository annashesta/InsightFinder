...
**ВАЖНО:** Если график подтвержден, ссылайся на него по его названию (например, "см. график boxplot_MonthlyRevenue"). Не вставляй изображения в отчет. Пользователь сам откроет файлы графиков.
...
*   Назови признак, который *наиболее сильно* различает группы согласно результатам анализа. Приведи конкретные значения (например, среднее значение признака в группе 1: X, в группе 0: Y). Проинтерпретируй, что это может означать в контексте исследования. Если есть график, обязательно сошлись на него.
...
*   **{stats_details}**
*   Сравни основные метрики (среднее, медиана, стандартное отклонение и прочие) по ключевым непрерывным признакам между группами. Акцентируй внимание на признаках с наибольшей разницей. Проинтерпретируй, что эти различия могут означать. Если есть график, обязательно сошлись на него.
...
*   Интерпретируй предоставленные графики (например, гистограммы, boxplots). Опиши форму распределений, наличие мод и визуально заметные различия между группами. Укажи, на какие графики стоит обратить внимание и почему. **Вставь названия ТОЛЬКО тех графиков, которые подтверждены данными инструментов.**

### 6. Выбросы и аномалии
*   **{outlier_summary}**
*   **{outlier_details}**
*   Сообщите о наличии выбросов, если они обнаружены. Укажите, в какой группе и на каких признаках они сконцентрированы, и как это может повлиять на анализ. **Названия графиков для этого раздела (если генерировались) также должны быть проверены и вставлены только при наличии подтверждения.**

### 7. Анализ взаимодействия признаков
*   **{interaction_summary}**
*   **{interaction_details}**
*   Опиши, если были обнаружены признаки, чье влияние на целевую переменную усиливается/ослабевает в зависимости от других факторов. **Если для иллюстрации были созданы графики (например, interaction plots), и они подтверждены, вставь их названия.** Если таких признаков нет или данные отсутствуют — пропусти этот пункт полностью.

### 8. Важность признаков (модель)
*   **{full_model_summary}**
*   **{model_details}**
*   Ранжируй признаки по их важности в предсказании принадлежности к группе, согласно результатам ML-модели. Проинтерпретируй, что это может означать. **Если есть подтвержденный график важности признаков (например, bar chart), вставь его название.**

### 9. Инсайт-ориентированные визуализации
{insight_viz_summary}
{insight_viz_details}
**Вставь ТОЛЬКО названия тех графиков, которые были созданы инструментом `InsightDrivenVisualizer` и подтверждены в его `details`.** Прокомментируй полученные графики.
