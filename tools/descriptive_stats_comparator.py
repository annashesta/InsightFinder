...
def descriptive_stats_comparator(df: pd.DataFrame, target_column: str, threshold_ratio: float = 0.2, **kwargs) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            return {"tool_name": tool_name, "status": "error", "summary": "", "details": {}, "error_message": "Target not binary"}
        ...
        for col in X_num.columns[:-1]:
            for stat in ['mean', 'median', 'std', 'min', 'max']:
                ...
                if rel_diff > threshold_ratio:
                    diffs[f"{col}_{stat}"] = {"group_0": float(val_0), "group_1": float(val_1), "relative_difference": float(rel_diff)}

        if not diffs:
            summary = f"Нет значимых различий > {threshold_ratio*100:.0f}%"
        else:
            top = next(iter(diffs))
            summary = f"Наибольшие различия у признака '{top.split('_')[0]}' по {top.split('_')[1]}."

        return {"tool_name": tool_name, "status": "success", "summary": summary,
                "details": {"threshold_ratio": threshold_ratio, "significant_differences": diffs, "n_features_with_diff": len(diffs)},
                "error_message": None}
