...
Запустите контейнер, пробросив порт 8502 (порт Gradio по умолчанию) и, опционально, подключив локальную директорию для сохранения логов:

```bash
# Запуск с сохранением логов на хосте
docker run -p 8502:8502 -v $(pwd)/logs:/app/logs insightfinder

# На Windows PowerShell для монтирования логов используйте:
# docker run -p 8502:8502 -v ${PWD}/logs:/app/logs insightfinder
```

> **Примечание:** При первом запуске папка `logs` будет создана автоматически. Монтирование позволяет логам сохраняться на вашем компьютере даже после остановки контейнера.
